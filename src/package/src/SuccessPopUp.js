...
function SuccessPopUp({ title, description, duration, isClosable, position }) {
            ...
            <>
                {createPortal(
                    <div ref={toogleRef} className={toggle === true ? [style.Bottom, style.showBottom].join(' ') : [style.Bottom, style.hideBottom].join(' ')}>
                        <Content animation={success} title={title} description={description} duration={duration} isClosable={isClosable} position={position} handleToggle={handleToggle} bcolor={'#48BB78'} />
                    </div>
                    , document.body)}
            </>
        )
